...
class P4DConv(nn.Module):
    def __init__(self,
        ...
        anchor_dim = 16
        self.anchor_point_mlp = nn.Sequential(
            nn.Conv1d(3 + in_planes, anchor_dim, kernel_size=1),
            nn.ReLU(inplace=True),
        )
        self.anchor_score = nn.Sequential(
            nn.Conv1d(anchor_dim * 2, 1, kernel_size=1),
            nn.Sigmoid(),
        )
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None,
                masks: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Args:
            xyzs: torch.Tensor
        ...
        if masks is not None:
            masks = torch.split(tensor=masks, split_size_or_sections=1, dim=1)
            masks = [torch.squeeze(input=mask, dim=1) for mask in masks]
        ...
        if self.temporal_padding_mode == 'zeros':
            ...
            if masks is not None:
                mask_padding = torch.zeros_like(masks[0])
                masks = [mask_padding] * self.temporal_padding[0] + masks
                masks = masks + [mask_padding] * self.temporal_padding[1]
        else:
            # 复制填充
            for i in range(self.temporal_padding[0]):
                xyzs = [xyzs[0]] + xyzs
            for i in range(self.temporal_padding[1]):
                xyzs = xyzs + [xyzs[-1]]
            if masks is not None:
                masks = [masks[0]] * self.temporal_padding[0] + masks
                masks = masks + [masks[-1]] * self.temporal_padding[1]
        ...
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):                 # temporal anchor frames
            ...
            # 为每个点预测重要性分数，选择 Top-K 高分点作为锚点。
            score_input = xyzs[t].transpose(1, 2)
            if self.in_planes != 0:
                score_input = torch.cat([score_input, features[t]], dim=1)
            point_feature = self.anchor_point_mlp(score_input)
            if masks is None:
                global_feature = point_feature.amax(dim=2, keepdim=True)
            else:
                valid = masks[t].unsqueeze(1)
                global_feature = point_feature.masked_fill(
                    ~valid, torch.finfo(point_feature.dtype).min
                ).amax(dim=2, keepdim=True)
                global_feature = torch.where(
                    valid.any(dim=2, keepdim=True),
                    global_feature,
                    torch.zeros_like(global_feature),
                )
            global_feature = global_feature.expand(-1, -1, npoints)
            point_scores = self.anchor_score(torch.cat(
                [point_feature, global_feature], dim=1
            )).squeeze(1)
            scores_for_topk = point_scores
            if masks is not None:
                scores_for_topk = scores_for_topk.masked_fill(~masks[t], -1.0)
            anchor_idx_long = torch.topk(
                scores_for_topk, npoints//self.spatial_stride, dim=1
            ).indices
            anchor_scores = torch.gather(point_scores, 1, anchor_idx_long)
            if masks is not None:
                anchor_scores = anchor_scores * torch.gather(
                    masks[t], 1, anchor_idx_long
                ).to(anchor_scores.dtype)
            anchor_idx = anchor_idx_long.int()
            ...
            # Top-K 索引不可导，让分数参与特征加权以训练锚点评分器。
            new_feature = new_feature * anchor_scores.unsqueeze(1)
...
class P4TransformerMlp(nn.Module):
    r"""
    ...
    def forward(self, model_input): 
        ...
        xyzs, features = self.tube_embedding(point_cloud, point_fea, mask)                                                                                   # [B, L, n, 3], [B, L, C, n]
...
if __name__ == "__main__":
    ...
    device = set_device(0)
    model = build_model('P4TransformerMlp').to(device)
    x = {
        'input': torch.zeros((1, 1, 300, 5), device=device),
        'mask': torch.ones((1, 1, 300), dtype=torch.bool, device=device),
    }
    profile_model("P4TransformerMlp", model, x)
