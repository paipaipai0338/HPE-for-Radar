...
def create_conv(
    ...
    Args:
        ...
        group (int): number of blocked connections for Conv3d/Conv2d
...
class SingleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,  stride=1, order='gcr', num_groups=8, padding=1, is3d=True, group=1):
...
class ResNetBlock(nn.Module):
    ...
    expansion = 1
    def __init__(self, in_channels, out_channels, stride=1, downsample=None, bn_type=None, bn_momentum=None, order='gcr', num_groups=8, is3d=True, group=1, **kwargs):
        ...
        if in_channels != out_channels:
            ...
            if is3d:
                self.conv1 = nn.Conv3d(
                    in_channels, out_channels, 1, groups=group
                )
            else:
        ...
        self.conv2 = SingleConv(out_channels, out_channels, kernel_size=kernel_size, stride=stride, order=order, num_groups=num_groups,
                                is3d=is3d, group=group)
        # remove non-linearity from the 3rd convolution since it's going to be applied after adding the residual
        n_order = order
        for c in 'rel':
            n_order = n_order.replace(c, '')
        self.conv3 = SingleConv(out_channels, out_channels, kernel_size=kernel_size, order=n_order,
                                num_groups=num_groups, is3d=is3d, group=group)
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, bn_type=None, bn_momentum=0.1, group=1
    ):
        super(BasicBlock, self).__init__()
        self.conv1 = nn.Conv3d(
        inplanes, planes, kernel_size=3, stride=stride, padding=1, groups=group, bias=False
        )
        ...
        inplanes, planes, kernel_size=3, stride=1, padding=1, groups=group, bias=False
        )
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv3d(
            inplanes, planes, kernel_size=1, groups=group, bias=False
        )
        self.bn1 = nn.BatchNorm3d(
            planes, momentum=bn_momentum
        )
        self.conv2 = nn.Conv3d(
            planes, planes, kernel_size=3, stride=stride, padding=1, groups=group, bias=False
        )
        self.bn2 = nn.BatchNorm3d(
            planes, momentum=bn_momentum
        )
        self.conv3 = nn.Conv3d(
            planes, planes * 4, kernel_size=1, groups=group, bias=False
        )
